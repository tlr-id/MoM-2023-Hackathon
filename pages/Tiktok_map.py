...
st.write('## 🔍 Tiktok Top Cities Statistics')
...
tiktok_audience_info = json_data['tiktok_audience_info']
...
for country in tiktok_audience_info['top_countries']:
    ...
    popup_insta = f"<br>{name}</br><strong>{country_name}</strong><br> Tiktok followers: {country_followers}</br><br> percentage_followers: {percentage_followers}</br>"
    make_circle_marker(location=location, radius=15, color='black',
                       fill_color='blue', map=insta_m, tooltip=country_code, popup=popup_insta, fill=True)
