...
st.write('## 🔍Instagram Top Cities Statistics')
...
instagram_audience_info = json_data['instagram_audience_info']
...
for country in instagram_audience_info['top_countries']:
    ...
    country_followers = country.get('followers')
    percentage_followers = country.get('percent')
    popup_insta = f"<br>{name}</br><strong>{country_name}</strong><br> Instagram followers: {country_followers}</br><br> percentage_followers: {percentage_followers}</br>"
    make_circle_marker(location=location, radius=10, color='purple',
                       fill_color='yellow', map=insta_m, tooltip=country_code, popup=popup_insta, fill=True)
